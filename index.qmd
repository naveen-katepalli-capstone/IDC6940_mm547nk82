...
```{r}
# Cluster Profiling Table: Summary Statistics by Cluster
library(dplyr)

# Create a summary table showing the mean of numeric variables per cluster
cluster_summary <- cleaned_data %>%
  ...
  summarise(
    ...
    state = NA_character_,  # Placeholder if state column is required for structure
    .groups = "drop"         # Prevents grouped tibble output
  )

# Display the full cluster summary table
